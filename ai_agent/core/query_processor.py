...
Если документ соответствует всем требованиям, подтвердите это."""
    ...
    def process_document_check(self, document_content: str, session_id: str) -> QueryResponse:
        ...
        try:
            ...
            relevant_chunks = self._get_relevant_context(search_query, top_k=10)
    ...
    def _get_relevant_context(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        """
        try:
            return self.document_manager.search_similar_chunks(query, top_k)
        except DocumentManagerError as e:
